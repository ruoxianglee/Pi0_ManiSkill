...
# 将四元数转换成 3 维轴角表示，用于构建状态向量
def _quat2axisangle(quat):
    """
    Copied from robosuite: https://github.com/ARISE-Initiative/robosuite/blob/eafb81f54ffc104f905ee48a16bb15f059176ad3/robosuite/utils/transform_utils.py#L490C1-L512C55
    """
    # 确保 quat 是 numpy 数组并转换为 float 类型
    quat = np.asarray(quat, dtype=float)
    # 提取并限制 w 分量在 [-1, 1]
    w = np.clip(quat[3], -1.0, 1.0)
    # 计算 sin(theta/2)
    sin_half_theta = np.sqrt(1.0 - w * w)
    if sin_half_theta < 1e-8:
        # 零度旋转
        return np.zeros(3, dtype=float)
    # 计算旋转轴
    axis = quat[:3] / sin_half_theta
    # 返回轴角表示（axis * angle），angle=2*arccos(w)
    return axis * (2.0 * np.arccos(w))
    ...
    def observation(self, observation: Dict):
        ...
        if self.include_state:
            ret["state"] = np.concatenate(
                                (
                                    observation["extra"]["tcp_pose"][:,:3][0], # dim: 3
                                    _quat2axisangle(observation["extra"]["tcp_pose"][:,3:][0]), # dim: 3
                                    observation["agent"]["qpos"][:,7:][0], # dim: 2
                                )
                            )
        if self.include_rgb and not self.include_depth:
            # images[x] has shape (1, 128, 128, 3)
            ...
            def thunk():
                ...
                if video_dir:
                    env = RecordEpisode(
                        ...
                        info_on_video=True,
                        source_type="Pi0_Libero",
                        source_desc="Pi0_Libero evaluation",
                    )
